...
PROCEDURE DIVISION.
    ...
    CALL 'UTIL' USING WS-STUDENT-ID, WS-STUDENT-NAME, WS-FUNC.
    DISPLAY B.
    ACCEPT a.
	...
	PERFORM UNTIL QUIT=1
		...
		IF USER-VAL='n'
		    DISPLAY CLRSCREEN
		    CALL 'UTIL' USING WS-STUDENT-ID, WS-STUDENT-NAME, WS-FUNC
		    DISPLAY SPACES LINE 20
			IF NB-VOL > VSTOP
				ADD 6 TO VSTART
				ADD 6 TO VSTOP
			ELSE
				DISPLAY "Derniere page atteinte" LINE 20 COL 1
			END-IF
		END-IF
		IF USER-VAL='p'
			DISPLAY SPACES LINE 20
			...
			ELSE
				DISPLAY "Première page atteinte" LINE 20 COL 1
			END-IF
		END-IF
	END-PERFORM.
...
PROCEDURE DIVISION USING LS-STUDENT-ID, LS-STUDENT-NAME, LS-FUNC.
